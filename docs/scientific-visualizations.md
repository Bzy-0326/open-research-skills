...
[返回仓库首页](../README.md) · [实际制作流程](#production) · [图件精修](#refinement) · [14-panel 数据图](#atlas) · [方法架构图](#diagram) · [其他任务](#recipes) · [交付检查](#delivery) · [常见问题](#faq)

`build-scientific-visualizations` 负责科学图件的组织、绘制、图注和实际渲染检查。可以只画一张图，也可以协调一篇稿件的整套图；不要求安装另外四个 skill。

这份手册先解释真实任务的材料、组织和交付，再用两个案例讲清不同的制作路径：**已有数据如何组织为复合图，已有方法如何组织为示意图。** 第一条可用随附脚本重跑，第二条提供可编辑资产和面向 agent 的完整任务示范。也涵盖会议图组、期刊主图与扩展图、现成图的局部修改和多轮迭代。
...
安装后在科研项目的 agent 对话中输入。Codex 可写 `$build-scientific-visualizations`，Claude Code 可写 `/build-scientific-visualizations`；下面的自然语言写法也明确指向同一 skill。

```text
使用 build-scientific-visualizations，制作论文 Figure 2。
材料是 results/、现有 render_figure2.py、figure2-notes.md 和 draft.md。
这张图要回答条件变化是否在 donor 层级保持一致，而不只是展示单细胞分布。
先检查源表字段、donor 与 cell 的层级及配对，再安排空间视图、分子比较和控制。
沿用已确定的分析，不重新选择样本或检验；缺失材料先标出。
目标插入宽度和导出要求见 journal-instructions.md。
...
一张图应能回答一个可说清楚的问题，例如“模型怎样利用扰动条件”“空间差异如何映射到分子层”“不同 donor 是否支持同一个方向”。然后逐面板说明输入来自哪份表、图像、结构或方法段落，哪些是观测、预测、示意和未来计划。

方法图需要区分数据流、条件关系和训练监督。结果图需要区分独立样本与嵌套观测、比较方向、单位、排除和配对。若正文说 donor-level，图注说 cell-level，先解决定义，不能让配色和排版替代科学判断。

### 2. 用阅读顺序决定面板面积

先确定视觉入口和证据转折，再分配大小。空间研究可能从组织总览到同源 ROI，再到 donor 汇总；模型论文可能从总体计算到关键模块，再到支持该设计的结果。不是所有面板都应等大，也不是越多越好。

已有明确的十四个面板需求时，先保留其科学作用并检查标签与图像空间，不能为了排得整齐直接删掉控制。若实际版面容纳不下，再依据证据依赖提出分图方案。`layout-sketch` 可以先讨论这个决定，不需要先制作每个精细图元。

```text
使用 build-scientific-visualizations，先完成 layout-sketch。
读取 figure-plan.md，保留其中所有证据角色和控制。
提出整页问题、视觉入口、分组、阅读顺序和相对面积，说明为何这样分配。
暂缺原图或数据的面板保留明确占位，不补造内容，不把草图当作最终图。
```

### 3. 分开制作数据面板与科学示意

数据图从原始或已确认的分析输出重绘，保留真实值、误差定义和比较单位。显微图保留源图、比例尺和裁切上下文；分子结构与基因组轨迹使用对应来源和坐标，不能从视觉相似的图片替代。

新建示意图采用后文的概念稿到原生 PPTX 路径。生成稿用于探索构图，准确文字、公式、模块和箭头必须回到方法材料核对。若当前期刊不允许相应用途的生成图像，应遵守其政策，不能借“后续矢量重绘”绕过限制。

### 4. 在实际插入尺寸下协调全图

同一个条件在不同面板使用相同颜色、名称与单位；颜色最好有形状、线型或直接标签作为第二个识别线索。热图的归一化范围、色标和缺失值应清楚，不能让“未测量”看起来像“数值为零”。

检查时使用论文中的真实宽度，不只看放大后的屏幕预览。轴标签、长分子名称、嵌套 ROI 边框、legend 和箭头尖端都需要可读；缩小到论文尺寸才出现的文字重叠仍是实际缺陷。

<a id="refinement"></a>

### 5. 精修与交付

先看整页层级，再看各科学对象，最后统一字号、线宽、间距和颜色。保留前一版，对照实际渲染决定修改是否更好；不是仅凭源码坐标或“检查通过”结束。局部要求可以直接局部修改，不重复整套构图。

```text
使用 build-scientific-visualizations，精修已有 Figure 1，不重新定义方法。
读取可编辑源文件、当前渲染、methods.md 和目标插入尺寸。
先检查整体阅读顺序，再处理局部科学图元，最后协调最终尺寸下的标签与线条。
只改发现问题的位置，保留前一版并比较渲染；没有改善时保留原版。
不改数据、运算关系或结果，交付可编辑修订、导出和简短修改说明。
```

三层精修的实际操作见随包[示意图精修流程](../skills/build-scientific-visualizations/references/visual-refinement.md)。
它适用于示意图和图组中的示意面板；数据图仍以来源和实际分析为准，不用视觉精修改变观测结果。

终稿要能说明源数据在哪里、怎样重绘、图注解释什么，以及哪些编辑能力已经验证。一个 PDF 中保留矢量，不等于每个对象在 PowerPoint 中都可编辑；把整张 PNG 放进 PPTX，也不等于完成原生重建。
...
```text
使用 build-scientific-visualizations，局部修改现有 Figure 3。
保留 render_figure3.py 中的数据选择、归一化与统计计算，只调整标签重叠、
panel c/d 对齐和 legend 的条件名称；不要重设计已经清楚的部分。
在输出副本保存修订，按论文插入尺寸对比前后图，并列出实际改动。
```

适合内容已经正确、只需提高可读性的图。若检查发现原有数据或样本定义有冲突，应指出原始位置，不能在绘图修改中悄悄重写分析。
...
先盘点现有图各自的论证工作：动机图是否提供了真实问题证据，框架图是否解释了方法差异，结果图是否能让人读懂比较。不要要求每篇稿件固定拥有这三类图；已有图重复同一信息时，应该调整分工。
...
主图承担核心证据顺序，Extended Data 和 Supplementary 承接细节、控制与稳健性。一起建立符号、条件名称、panel lettering 和图注习惯，但不把所有图机械排成同一种网格。正文、图注和数据表之间的样本数应能互相解释。
...
| 蛋白、代谢与糖生物 | 丰度、检测率、鉴定等级及缺失原因 | 展示分子层比较，不混淆未检测与低丰度 |
| 基因组与调控 | 参考版本、坐标、方向与轨迹来源 | 让局部调控证据处于准确的基因组上下文 |
| 结构与分子 | 实际结构、构象和注释来源 | 表达真实几何及关系，而非生成猜测结构 |
| 临床或队列 | 纳入、随访、终点、患者与重复测量关系 | 解释样本形成与比较，不夸大独立样本量 |

如果几个模态没有在同一个体或样本中配对，图中就不能画成已经证实的一对一跨模态连接。可以展示各自证据和明确标注的假设，但必须让读者看出区别。
...
| 产物 | 需要能观察到的结果 |
|---|---|
| 最终图件 | 在实际插入宽度可读，长标签和比例尺未被裁掉 |
| 源代码与数据 | 能说明输入、处理和面板映射；提供可复用的运行方法 |
| 原生 PPTX（示意图路线） | 文字、模块和连线能分别选择；保存与重开后仍正确 |
| PDF / SVG | 格式与尺寸正确，文字和需要保留的矢量对象未被整页栅格化 |
| Legend | 面板顺序、样本数、独立单位、配对、误差、单位与统计来源一致 |
| Source data | 读者能定位每个定量面板的依据，受限资料使用合法访问说明 |
| 简短交付说明 | 哪些检查已完成，使用了什么字体或渲染替代，哪些兼容性未验证 |

隐私或受限原始数据不因“交付源数据”而自动公开。先确定交付对象是内部合作者、审稿人还是公众，再提供对应允许的文件。内部工作记录、生成提示和工具运行过程不进入论文或公开图件附件。

收到文件后，优先打开实际成品：修改一个标签、移动一个模块、按给定命令重绘一个数据面板，能比单看文件扩展名更直接地判断可复用程度。无需为简单局部改图再建立整套独立验收系统。
